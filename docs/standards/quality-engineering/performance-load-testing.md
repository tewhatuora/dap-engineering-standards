...
Performance & Load Testing defines requirements for verifying that a service meets its defined performance and capacity targets under realistic and adverse demand. It directs engineering teams toward representative, repeatable testing that surfaces a capacity constraint or performance regression while it remains cheap to address.

## Standards

### Performance Test Types

These requirements identify when each performance test type should be considered, based on a service's defined targets and demand pattern.
...
These requirements identify the events that trigger performance testing, and require it to be repeated at a regular interval as a service evolves.
...
These requirements validate a service against its defined performance and capacity targets, using a workload that reflects realistic demand.
...
These requirements keep the environment and data used for performance testing representative of production, so its result can be relied on.
...
These requirements constrain how and where a performance test executes, and how it treats an external dependency.
...
These requirements define the telemetry captured during a performance test's execution.
...
These requirements govern how a performance test's result is evaluated and acted on.
