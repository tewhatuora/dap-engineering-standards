...
These requirements set out how a function's configuration is defined as code and remains the authoritative source for what is deployed.
...
These requirements address how a function's timeout, memory allocation, and cold-start latency are set from its own workload's needs, rather than left at a platform default.
...
These requirements describe how a function's implementation language is chosen, and its cold-start overhead optimised, to meet a path's latency and cost needs.
...
These requirements cover how a function's concurrency is bounded so a downstream dependency is not overwhelmed by its own scale-out.
...
These requirements guide how a failed invocation is safely retried, or otherwise recovered once retries are exhausted.
...
These requirements set out how an invocation is correlated end-to-end and how its cost contribution is made visible.
...
These requirements address how a function's code and packaging remain portable to a different platform or runtime model.
...
These requirements cover how a function's shared dependencies are packaged separately, and its reusable resources are initialised for efficiency.
...
These requirements guide how a function's scope stays generic enough to avoid unnecessary duplication.
...
These requirements describe how a function's published code remains identifiable and immutable once deployed.
...
These requirements cover how a function's permissions remain scoped to only what its own code currently requires.
...
These requirements guide how a function's secrets are kept secure and current at runtime.
