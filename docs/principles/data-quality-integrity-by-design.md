...
Data Quality & Integrity by Design establishes the expectation that data is correct, complete, consistent, and free of unresolved duplication or contradiction throughout its lifecycle, rather than assumed accurate because it exists in a system of record. It directs engineering effort toward validating data close to its source, enforcing integrity constraints at the data layer, and detecting and correcting quality issues as they arise, so that decisions, reporting, and downstream systems can rely on the data they consume.

## Principles

### Defining Data Quality Expectations as Part of Design

These principles specify a data item's quality expectations as part of a service's design, before implementation begins.
...
These principles validate data as close to its point of entry as possible, rather than relying on a downstream process.
...
These principles enforce structural and referential integrity at the data layer, or by the responsible service where the store cannot.
...
These principles prevent a duplicate or conflicting record from persisting unresolved.
...
### Traceability & Lineage of Data Changes

These principles keep a change to data used in an operational, financial, or clinical decision traceable to its source, timing, and responsible actor.
...
### Detecting Data Quality Issues Through Monitoring

These principles detect data quality degradation through ongoing monitoring, rather than assuming it absent after initial validation.
...
These principles correct a confirmed data quality issue at its root cause and track it to completion.
...
These principles verify a validation or quality rule through automated testing rather than manual inspection alone.
