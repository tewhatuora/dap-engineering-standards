...
These principles set out how a component's verification approach is decided as part of its design.
...
These principles describe how an external dependency becomes substitutable with a test double, rather than hard-coded or global.
...
These principles address how non-deterministic factors are controlled so a test produces the same result on every execution.
...
These principles set out how a component exposes its outcome through its interface, so it is verified without inspecting internals.
...
These principles cover how automated test coverage is sized to a change's risk, criticality, and complexity.
...
These principles describe how a service supports execution against controlled test data in an environment isolated from production.
...
These principles guide how a non-functional characteristic is designed to be verifiable through automated testing, not assumed from implementation.
