...
Interoperability by Design establishes the expectation that systems and services are designed to exchange data and functionality with other systems predictably and consistently, rather than treating integration as an incidental concern addressed only once a specific integration is requested. It directs engineering effort toward well-defined interfaces, compatible data exchange, and preference for open standards, so that systems can be integrated, extended, and evolved without accumulating brittle or one-off integrations.

## Principles

### Identifying Integration Needs During Design

These principles identify which systems a service will exchange data or functionality with, during design.
...
### Well-Defined, Documented Interfaces and Contracts

These principles expose what a service shares with other systems through an explicit, documented, discoverable interface.
...
These principles prefer an open, widely-adopted standard over a proprietary alternative, where one exists.
...
These principles preserve compatibility for existing consumers as a published interface changes.
...
These principles keep a data format and its meaning consistent and reconciled across systems.
...
### Designing for Consumers Beyond the Immediate Integration

These principles design an interface to support future consumers, not only the one it was first built for.
...
### Minimising Point-to-Point Integration Complexity

These principles prefer a shared, reusable integration mechanism over repeated bespoke integrations.
...
### Verifying Interoperability Through Contract & Integration Testing

These principles verify an integration point through contract or integration testing rather than assuming it works.
